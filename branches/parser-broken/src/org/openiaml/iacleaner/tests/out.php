...
// make sure that the db exists for stored_events$line_break$$db = new PDO('sqlite:stored_events.db') or local_die("could not open db");
 ...
 */
        // first, make sure the table exists$line_break$$db = new PDO('sqlite:stored_events.db') or local_die("could not open db");
        $s = $db->prepare("SELECT * FROM stored_events");
        if (!$s) {
          // create the table
          $q = $db->query("CREATE TABLE stored_events (
        ...
        // delete all the events for this page now$line_break$log_message("deleting all events for page visual.11e37b8d948.d");
