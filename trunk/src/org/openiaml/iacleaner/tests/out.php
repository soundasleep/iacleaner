...
/* make sure that the db exists for stored_events*/$db = new PDO('sqlite:stored_events.db') or local_die("could not open db");
...
do_model_11e47202c05_19( /* a property from our session */
...
<html>
  <head>
    ...
    <script language="Javascript">
      /* approach #2: use event queues stored on the server, if the function is not available on the current page */
    ...
    <script language="Javascript">
        ...
        if (running_model_11e47202c05_20 == false) {
          ...
          if (try_catch_depth == 0 ) {
            try {
              ...
              /* chained operation setPropertyToValue */
              /* outFlows first to = org.openiaml.model.model.impl.ApplicationElementPropertyImpl@13641d6 (isGenerated: false, id: model.11e37b8d89c.17, name: my login key, defaultValue: null)*/
              set_session("model_11e37b8d89c_17", "null" /* static value "model_11e47202c05_21" */
              , function_queue);
      ...
      function loadStoredEvents() {
        ...
        <?php /** * Expand all possible events for the current page. */
        /* first, make sure the table exists*/$db = new PDO('sqlite:stored_events.db') or local_die("could not open db");
        $s = $db->prepare("SELECT * FROM stored_events");
        if (!$s) {
          /* create the table*/
          $q = $db->query("CREATE TABLE stored_events (
        ...
        /* delete all the events for this page now*/log_message("deleting all events for page visual.11e37b8d948.d");
  ...
  <body onLoad="loadStoredEvents()">
    ...
    <?php /* expand any fail error messages */
    if (require_get("fail")) {
      ?>
      <div class="error">
